    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            <ToolsProps
                ...
                icon={<svg className="w-8 h-8 sm:w-10 sm:h-10 text-indigo-600 dark:text-indigo-400" viewBox="0 0 24 24" fill="none">
            ...
            <ToolsProps
                ...
                icon={<svg className="w-8 h-8 sm:w-10 sm:h-10" viewBox="0 0 54 54" fill="none">
...
function ToolsProps(props) {
    ...
    return (
        <div className={`group border-2 border-slate-400 ${props.hvrColor} bg-slate-50/50 rounded-2xl p-5 shadow-sm hover:shadow-md dark:border-slate-800 dark:bg-slate-900/50 transition-all flex flex-col min-h-40 sm:min-h-56 justify-between`}>
            <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-4">
                <span className="text-2xl">{props.icon}</span>
                <h3 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-slate-100">{props.title}</h3>
            </div>
            <div className="mb-6">
                <p className="text-sm md:text-base text-slate-500 dark:text-slate-400">{props.desc}</p>
            </div>
            <button
                onClick={() => navigate(props.to)}
                className={`w-full py-2.5 rounded-lg text-white text-xs sm:text-sm font-semibold flex items-center justify-center gap-1.5 transition-all shadow-sm ${props.btnColor}`}
            >
                {props.btnText}
