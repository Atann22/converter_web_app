...
export default function CompressPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-12">
            <h1 className="text-lg sm:text-3xl font-bold mb-1 text-slate-800 text-center">Compress Image</h1>
            <p className="text-xs mb-5 sm:text-sm font-medium text-slate-600 text-center">Upload an image and reduce its file size in seconds.</p>
                ...
                <div className="flex flex-col justify-center pt-6 md:pt-4 border-t md:border-t-0 md:border-l md:pl-8 border-slate-100">
                    {!resultUrl ? (
                        <div>
                            <div className="relative mb-5 sm:mb-6 inline-block w-full">
                                <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs sm:text-sm font-medium text-slate-500 shadow-sm outline-none hover:border-slate-300 transition-all flex items-center justify-between">
                                    <span className={quality ? "font-bold text-slate-700" : ""}>
                                        ...
                                        <div className="absolute left-0 mt-1.5 w-full bg-white rounded-xl shadow-xl border border-slate-100 py-1.5 z-20 text-center">
                                            {qualityOptions.map((opt) => (
                                                <button
                                                    ...
                                                    className={`w-full text-center px-4 py-2.5 text-xs sm:text-sm font-bold transition-all block ${quality === opt
                            ...
                            <button
                                ...
                                className={`w-full py-3 rounded-xl font-medium text-white text-xs sm:text-sm transition-all ${(!file || !quality) ? 'bg-slate-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-100'}`}
                            >
                                Compress
                            </button>
                        </div>
                        ...
                        <div className="flex flex-col gap-3.5 w-full max-w-60 mx-auto items-center">
                            <button type="button" onClick={handleResetAll} className="w-full bg-white hover:bg-slate-50 text-slate-500 font-medium py-3 px-4 border border-slate-200 rounded-xl text-xs sm:text-sm transition-all text-center shadow-sm leading-tight">Compress Another Image</button>
                            <a
                                href={resultUrl || "#"}
